...
## 负责处理场景中所有可交互物件的通用逻辑。
## 核心功能：
## 1. [物理]：根据材质类型自动配置物理属性 (弹性/摩擦)。
## 2. [受击]：处理生命值扣除、死亡销毁。
## 3. [交互]：支持引力枪的“连根拔起”视觉效果。
## 4. [掉落]：死亡时根据配置生成掉落物 (如木头、石头)。

#region 1. 基础配置与枚举
# 定义材质类型，用于 ERS 系统或物理反馈识别
enum ObjectMaterial {
	WOOD,   ## 木头：低弹力，易碎
	STONE,  ## 石头：无弹力，坚硬，重
	METAL,  ## 金属：无弹力，极硬
...
@export var object_name: String = "Object"                  ## 物件名称 (用于日志或UI)
...
@export var is_movable_by_default: bool = false             ## 初始状态：是否受物理引擎控制 (false=静止)
@export var mass_override: float = 10.0                     ## 质量覆盖 (影响击退效果)

@export_group("Visual Effects")
@export var jelly_strength: Vector2 = Vector2(1.2, 0.8)     ## [保留] 果冻特效强度
@export var jelly_duration: float = 1.0                     ## [保留] 果冻特效持续时间

@export_group("Loot Settings")
@export var loot_table: Array[LootData] = []                ## 掉落表配置 (需包含 PickupItem 的预制体)
...
#region 3. 内部状态变量
# --- 基础状态 ---
var default_scale: Vector2       ## [内部] 初始缩放
var default_pos: Vector2         ## [内部] 初始位置 (相对于父节点)
var original_sprite_pos: Vector2 ## [内部] Sprite 的原始局部坐标 (用于震动复位)
var current_tween: Tween         ## [内部] 通用动画控制器 (受击、恢复等)

# --- 引力枪系统变量 ---
var is_under_gravity: bool = false ## [状态] 当前是否正被引力枪吸住
var gravity_pull_tween: Tween      ## [动画] 专门控制“连根拔起”流程的 Tween
var is_shaking: bool = false       ## [状态] 是否进入“抖动”阶段
var shake_timer: float = 0.0       ## [内部] 抖动计时器
var is_dying: bool = false         ## [状态] 防止死亡逻辑多次触发
#endregion
...
func _ready() -> void:
	...
	# 2. 设置初始移动状态 (使用 call_deferred 避免物理引擎初始化冲突)
...
## 切换移动性 (Active=true 启用物理, Active=false 冻结)
...
func take_damage(amount: int, _attacker_type: int, attacker_node: Node2D = null) -> void:
	...
	# 3. 视觉反馈 (可选：震荡效果)
	if attacker_node and not is_dying:
		var hit_dir = (global_position - attacker_node.global_position).normalized()
		trigger_shockwave_shake(hit_dir)
#endregion

#region 6. 引力枪交互逻辑

## [入口] 应用引力视觉效果 (每帧由武器脚本调用)
...
func _instantiate_item(item_scene: PackedScene):
	...
	# [修复] 必须立即 add_child，否则 create_tween 会失败
	# 如果担心物理冲突，可以使用 get_tree().current_scene.add_child(item)
	# 这里假设 get_parent() 是场景容器 (Level)
	...
	# [关键] 调用掉落物的 launch 启动动画
	# 此时 item 已经在树里了，launch 里的 Tween 才能正常工作
